...
| `product_questions` | «خریدارها چی پرسیدن؟» — پرسش‌ها با جواب‌ها، مشخص شده کی جواب داده |
...
- سرچ دیجی‌کالا **کلمه‌ها رو OR می‌کنه** — نتیجه‌ای که فقط بخشی از کوئری رو داشته باشه با `low_confidence` فلگ می‌خوره (با لیست دقیق `unmatched_terms`)، و صفحه‌هایی که بی‌سروصدا سقف می‌خورن خودشون رو با `page_clamped` می‌گن.
...
**سرویس عمومی رایگان** روی Cloudflare Workers. **۶۰ درخواست در دقیقه برای هر IP** روی POST /mcp (۴۲۹ با `retry-after` وقتی رد بشه؛ شمارنده مال سرویس ریت‌لیمیت کلاودفلره و به‌ازای هر دیتاسنتره). یه جلسه‌ی عادی ایجنت هیچ‌وقت بهش نمی‌رسه — این سقف فقط جلوی سوءاستفاده‌ی ترکانی رو می‌گیره. چیزی که سرور همچنان پِیس می‌کنه درخواست‌های خودش به دیجی‌کالاست: با فاصله‌ی نیم‌ثانیه و backoff وقتی دیجی‌کالا مقاومت کنه، پس استفاده‌ی عادی هیچ‌وقت شکل هجوم به خود نمی‌گیره.
