...
- **بدون دیتای کاربر.** هیچ‌چیز از تو ذخیره نمی‌شه. سرور فقط دو چیز نگه می‌داره: یه کش کوتاه‌مدت جواب‌ها (چند دقیقه) و کوکی چالش امنیتی خود دیجی‌کالا (۱۰ دقیقه)، که یه بار حلش بقیه نمونه‌ها رو هم معاف می‌کنه. قیمت، موجودی و تخفیف هر بار که کش منقضی بشه دوباره از دیجی‌کالا خونده می‌شن.
...
**سرویس عمومی رایگان** روی Cloudflare Workers. **مصرف منصفانه: دقیقه‌ای ۶۰ درخواست برای هر IP** (با `retry-after` وقتی رد بشی) — استفاده عادی هیچ‌وقت بهش نمی‌رسه.
